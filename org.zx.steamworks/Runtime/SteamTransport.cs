...
#region Internal Object Model
        ...
        private readonly Dictionary<ulong, SteamConnectionData> connectionMapping = new();

        private readonly Queue<SteamNetConnectionStatusChangedCallback_t> connectionStatusChangeQueue = new();
        ...
        public override void DisconnectLocalClient()
        {
            ...
            {
                NetworkLog.LogInfoServer(nameof(SteamTransport) + " - DisconnectLocalClient");
            }
        ...
        public override void DisconnectRemoteClient(ulong clientId)
        {
            if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
            {
                NetworkLog.LogInfoServer(nameof(SteamTransport.DisconnectRemoteClient) + " - clientId: " + clientId);
            }
            ...
            if (!connectionMapping.ContainsKey(clientId))
            {
                ...
                {
                    NetworkLog.LogErrorServer(nameof(SteamTransport) +
                                              " - Can't disconect client, client not connected, clientId: " + clientId);
                }
        ...
        public override ulong GetCurrentRtt(ulong clientId)
        {
            if (isServer)
            {
                ...
                else
                {
                    ...
                    {
                        NetworkLog.LogErrorServer(nameof(SteamTransport) +
                                                  " - Can't GetCurrentRtt from client, client not connected, clientId: " +
                                                  clientId);
                    }
                }
            }
        ...
        public override void Initialize(NetworkManager? networkManager = null)
        {
            if (!IsSupported)
            {
                if (NetworkManager.Singleton.LogLevel <= LogLevel.Error)
                {
                    NetworkLog.LogErrorServer(nameof(SteamTransport) + " - Initialize - Steamworks.NET not ready, " +
                                              nameof(SteamTransport) + " can not run without it");
                }
...
#region Connnection State Changes
            ...
            while (connectionStatusChangeQueue.Count > 0)
            {
                    ...
                    ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_Connecting)
                {
                    ...
                    if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
                    {
                        NetworkLog.LogInfoServer(nameof(SteamTransport) + " - connection request from " +
...
#if UNITY_SERVER
                        if ((res =
 SteamGameServerNetworkingSockets.AcceptConnection(param.m_hConn)) == EResult.k_EResultOK)
#else
...
#endif
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    Debug.Log(
                                        $"Connection {param.m_info.m_identityRemote.GetSteamID64()} could not be accepted: this is not a server");
                                }
                            }
                        }
                        else
                        {
                            ...
                            {
                                Debug.Log(
                                    $"Connection {param.m_info.m_identityRemote.GetSteamID64()} could not be accepted: {res}");
                            }
                        }
                    }
                }
                         ...
                         ESteamNetworkingConnectionState.k_ESteamNetworkingConnectionState_Connected)
                {
                    if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
                    {
                        Debug.Log(nameof(SteamTransport) + " - connection request to " +
                                  param.m_info.m_identityRemote.GetSteamID64() + " was accepted!");
                    }
                            ...
                            .k_ESteamNetworkingConnectionState_ProblemDetectedLocally)
                {
                    ...
                    if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
                    {
                        Debug.Log(nameof(SteamTransport) +
                                  $" - connection closed for {param.m_info.m_identityRemote.GetSteamID64()} state responce: {param.m_info.m_eState}");
                    }
                ...
                else
                {
                    ...
                    {
                        Debug.Log(
                            $"Connection {param.m_info.m_identityRemote.GetSteamID64()} state changed: {param.m_info.m_eState}");
                    }
                }
            }
...
#if UNITY_SERVER
                if ((messageCount =
 SteamGameServerNetworkingSockets.ReceiveMessagesOnConnection(connectionData.connection, ptrs, 1)) > 0)
#else
                if ((messageCount =
                        SteamNetworkingSockets.ReceiveMessagesOnConnection(connectionData.connection, ptrs, 1)) > 0)
#endif
                {
                    if (messageCount > 0)
                    {
        ...
        public override void Send(ulong clientId, ArraySegment<byte> segment, NetworkDelivery delivery)
        {
            ...
            if (connectionMapping.ContainsKey(clientId))
            {
                ...
                var sendFlag = delivery switch
                {
                    NetworkDelivery.Reliable or NetworkDelivery.ReliableFragmentedSequenced => Constants
...
#if UNITY_SERVER
                EResult responce =
 SteamGameServerNetworkingSockets.SendMessageToConnection(connection, pData, (uint)data.Length, sendFlag, out long _);
#else
                EResult responce = SteamNetworkingSockets.SendMessageToConnection(connection, pData, (uint) data.Length,
                    sendFlag, out long _);
#endif
            ...
            else
            {
                ...
                if (NetworkManager.Singleton.LogLevel <= LogLevel.Error)
                {
                    ...
                    NetworkLog.LogErrorServer(nameof(SteamTransport.Send) +
                                              " - Trying to send on unknown connection: " + clientId);
                }
            }
        }
 ...
 Callback<SteamNetConnectionStatusChangedCallback_t>.CreateGameServer(OnConnectionStatusChanged);
            ...
            if (SteamLobbyManager.Instance.Room == null)
            {
                throw new Exception("SteamLobbyManager.Instance.Room == null");
            }

            var owner = SteamMatchmaking.GetLobbyOwner(SteamLobbyManager.Instance.Room.Value);

            serverUser = new SteamConnectionData(owner);
...
#if UNITY_SERVER
                serverUser.connection =
 SteamGameServerNetworkingSockets.ConnectP2P(ref smi, 0, options.Length, options);
#else
                serverUser.connection = SteamNetworkingSockets.ConnectP2P(ref smi, 0, options.Length, options);
#endif
                connectionMapping.Add(owner.m_SteamID, serverUser);
        ...
        public override void Shutdown()
        {
            if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
            {
                UnityEngine.Debug.Log(nameof(SteamTransport.Shutdown));
            }
            ...
            {
                //Delay
                NetworkManager.Singleton.StartCoroutine(Delay(0.1f, () => { CloseP2PSessions(); }));
            }
            else
            {
                CloseP2PSessions();
        ...
        private void CloseP2PSessions()
        {
            ...
            foreach (var user in connectionMapping.Values)
            {
#if UNITY_SERVER
            ...
            if (NetworkManager.Singleton.LogLevel <= LogLevel.Developer)
            {
                UnityEngine.Debug.Log(nameof(SteamTransport) +
                                      " - CloseP2PSessions - has Closed P2P Sessions With all Users");
            }
